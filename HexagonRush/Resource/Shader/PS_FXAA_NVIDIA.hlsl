...
{
    return gOriginMap.SampleLevel(Sam_Clamp, pos.xy, 0.0);
}

float4 FxaaTexGrad(float2 pos, float2 grad)
{
    return gOriginMap.SampleGrad(Sam_Clamp, pos.xy, grad, grad);
}

float4 FxaaTexOff(float2 pos, int2 off)
{
    return gOriginMap.SampleLevel(Sam_Clamp, pos.xy, 0.0, off.xy);
}
...
----------------------------------------------------------------------------*/
    float3 rgbN = FxaaTexOff(pos.xy, FxaaInt2(0, -1)).xyz;
    float3 rgbW = FxaaTexOff(pos.xy, FxaaInt2(-1, 0)).xyz;
    float3 rgbM = FxaaTexOff(pos.xy, FxaaInt2(0, 0)).xyz;
    float3 rgbE = FxaaTexOff(pos.xy, FxaaInt2(1, 0)).xyz;
    float3 rgbS = FxaaTexOff(pos.xy, FxaaInt2(0, 1)).xyz;
...
----------------------------------------------------------------------------*/
    float3 rgbNW = FxaaTexOff(pos.xy, FxaaInt2(-1, -1)).xyz;
    float3 rgbNE = FxaaTexOff(pos.xy, FxaaInt2(1, -1)).xyz;
    float3 rgbSW = FxaaTexOff(pos.xy, FxaaInt2(-1, 1)).xyz;
    float3 rgbSE = FxaaTexOff(pos.xy, FxaaInt2(1, 1)).xyz;
...
#endif
    float lengthSign = horzSpan ? -_textureInfo.w : -_textureInfo.z;
    if (!horzSpan)
        lumaN = lumaW;
    if (!horzSpan)
        lumaS = lumaE;
...
--------------------------------------------------------------*/
    ...
    float2 offNP = horzSpan ?
        FxaaFloat2(_textureInfo.z, 0.0) :
        FxaaFloat2(0.0f, _textureInfo.w);
    ...
    for (int i = 0; i < FXAA_SEARCH_STEPS; i++)
    {
        ...
        if (!doneN)
            lumaEndN =
                FxaaLuma(FxaaTexLod0(posN.xy).xyz);
        if (!doneP)
            lumaEndP =
                FxaaLuma(FxaaTexLod0(posP.xy).xyz);
#else
            if(!doneN) lumaEndN = 
...
/*----------------------------------------------------------
         CHECK IF PIXEL IS IN SECTION OF SPAN WHICH GETS NO 
	    FILTERING
------------------------------------------------------------
...
--------------------------------------------------*/
    ...
    float subPixelOffset = (0.5 + (dstN * (-1.0 / spanLength))) * lengthSign;
        ...
        float ox = horzSpan ? 0.0 : subPixelOffset*2.0/_textureInfo.z;
        float oy = horzSpan ? subPixelOffset*2.0/_textureInfo.w : 0.0;
...
#endif
    float3 rgbF = FxaaTexLod0(FxaaFloat2(
...
{
    return float4(FxaaPixelShader(input.uv), 1.0f);
}