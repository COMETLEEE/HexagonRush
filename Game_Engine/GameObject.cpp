...
namespace Muscle
{
	...
	std::shared_ptr<GameObject> CreateFromPrefab(const tstring& prefabName)
	{
		std::shared_ptr<Prefab_ASE> prefabASE = ResourceManager::Get()->GetPrefab(prefabName);

		assert(prefabASE);

		// 프리팹을 통해서 게임 오브젝트를 만들고
